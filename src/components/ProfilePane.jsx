    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur">
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
        <img
          src={user.avatar}
          alt={user.name}
          className="h-24 w-24 rounded-2xl ring-2 ring-sky-500/40"
        />
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-2xl font-semibold text-white">{user.name}</h2>
            {user.badge && (
              <span className="inline-flex items-center gap-1 rounded-full bg-sky-500/15 px-2 py-1 text-xs font-medium text-sky-300 ring-1 ring-sky-400/30">
